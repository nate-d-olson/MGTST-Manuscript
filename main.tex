...
Analysis of 16S rRNA marker-gene surveys may be performed by numerous bioinformatic pipelines and downstream analysis methods.
However, there is limited guidance on how to decide between methods. 
Therefore, appropriate datasets and statistics for assessing these methods are needed.
Mixtures of environmental samples have been used to assess other sequencing methods such as RNAseq, but not 16S rRNA sequencing.
With mixture datasets expected values for assessment are calculated using information from the unmixed samples and mixture design.


\parttitle{Results}
We developed an assessment framework for evaluating 16S rRNA marker-gene survey analysis methods. 
The framework consists of a two-sample titration mixture dataset and metrics for evaluating count table qualitative and quantitative characteristics. 
The qualitative assessment evaluates feature presence/ absence by testing if random sampling can explain observed relative abundance of features only present unmixed samples and titrations.
The quantitative assessment evaluates how well relative and differential abundance values agree with the expected values overall and on the feature-level.
To demonstrate our assessment framework we evaluated count tables generated by three commonly used bioinformatic pipelines
i) DADA2 a sequence inference method, ii) Mothur a \textit{de novo} clustering method, and iii) QIIME with open-reference clustering.
The qualitative assessment indicated that the vast majority of Mothur and QIIME features specific to unmixed samples and titrations were explained by random sampling alone but not DADA2 features.
When combined with count table sparsity these results indicate that for the three pipelines DADA2 has a higher false negative rate whereas Mothur and QIIME have higher false positive rates.
The quantitative assessment indicated that overall the observed relative abundance and differential abundance values were consistent with the expected values for all three pipelines.
However, feature-level analysis indicated identified subset of poor performing for all pipelines. 
The bais responsible for these poor performing features could not identified, indicating a potentially unknown and unaccounted for bias. 

\parttitle{Conclusions}
We developed a novel framework for assessing 16S rRNA marker-gene survey analysis methods.
...
%% I still don't think this fits here but one of the reviewers said this should go in the intro
...
%% Add statement / analysis for degree of difference between PRE and POST by individual???
