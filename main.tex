...
\title{A framework for assessing 16S marker gene survey data analysis methods using mixtures of human stool sample DNA extracts.}
...
However, there is limited guidance on how to decide between methods, appropriate datasets and statistics for assessing these methods are needed.
To address this need we developed an assessment framework for evaluating 16S rRNA marker-gene survey analysis methods. 
The assessment framework includes a mixture dataset as well as qualitative and quantitative assessment metrics. 
Mixtures of environmental samples have been used to evaluate other count based sequencing methods such as RNAseq.
By using mixtures one can calculate expected values for assessment based on information from the unmixed samples and the mixture design.
We developed a set of metrics that make use of the expected values to assess the qualitative and quantitative characteristics of 16S rRNA marker-gene survey analysis methods, specifically the bioinformatic pipelines used to generate count tables. 
To demonstrate how the assessment framework is used we evaluated count tables generated using three commonly used bioinformatic pipelines
...
\parttitle{Results}
The assessment framework consists of a mixture dataset and metrics evaluate count table qualitative and quantitative characteristics. 
A two-sample titration mixture design was used for the assessment dataset. 
...
From our assessment framework demonstration we found that the abundance of Mothur and QIIME features specific to unmixed samples and titrations were explained by sampling alone.
...
%% Note diff abundance figures moved to supplemental

%% Some of this text should be moved to the discussion and maybe supplemental material
The agreement between log-fold change estimates and expected values were
individual specific and consistent across pipelines (Fig. S9A). The individual specific effect was attributed to
...
(Fig. S9B). There was a long tail of high error
...
compare pipeline performance (Fig. S10). Similar to
...
0.08, Fig. S10B, Variance: F = 47.39, 0.23, p = 0, 0.8, Fig.
S10C).
