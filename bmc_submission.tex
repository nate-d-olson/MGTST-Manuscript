...
We developed a mixture dataset with real data complexity and an expected value for assessing 16S rRNA bioinformatic pipelines and downstream analysis methods. 
...
\parttitle{Results}
The qualitative assessment was used to evalute features only present in unmixed samples and titrations. 
...
To assess the qualitative characteristics of the 16S rRNA sequencing
measurement process mock communities are commonly used
\cite{bokulich2016mockrobiota}. As the number of organisms in the mock
community is known, the total number of features can be compared to the
expected value. The number of observed features in a mock community is
...
inference methods aim to reduce the number of sequence artifact
features. While mock communities have an expected number of features and
composition, they lack the feature diversity and relative abundance
dynamic range of real samples \cite{bokulich2016mockrobiota}.
...
abundance estimate quantitative accuracy \cite{Kopylova2014}. Results
...
compared to Gram-positive bacteria \cite{Costea2017, Olson2012}.
...
been observed \cite{Pinto2012, Hansen1998, Gohl2016}. Simulated count
tables have been used to assess differential abundance method (fold
change differences in relative abundance), where specific taxa are
artificially overrepresented in one set of samples compared to another
\cite{McMurdie2014}. Using simulated data to assess log fold-change
...
precision, agreement with other methods, can be evaluated. By mixing
...
across pipelines, but the qualitative results varied by pipeline. We
have made both the dataset and metrics developed in this study publicly
available for evaluating bioinformatic pipelines.
...
\subsection*{Measurement Assessment}
...
\ref{fig:qualPlot}B). For mixture datasets low abundance features
...
bias were all significantly different from each other
...
and DADA2 = 1 (Fig. \ref{fig:relAbuErrorMetrics}B). We evaluated whether
...
titrations were included and they were not monotonically decreasing and
therefore resulted in unrealistic expected log fold-change values, e.g.,
...
notable exception. For E01JH0011 the Mothur log fold-change estimates
...
log-fold change error distributions for log-fold change estimates using
...
to a bias associated with the molecular laboratory portion of the
measurement process and not the bioinformatic pipelines. Exploratory
...
\ref{fig:logFCerror}C). We also evaluated whether poor feature-level
...
and differential abundance assessments. Outlier features are not likely
a pipeline artifact as they were observed in count tables generated
using the unclustered pipeline as well as standard bioinformatic
pipelines. We were unable to attribute outlier features to relative
abundance values, log fold-change between unmixed samples, and sequence
GC content. Features with extreme metric values were not limited to any
specific taxonomic group or phylogenetic clade. Outlier features could
not be attributed to bioinformatic pipelines and are likely due to
biases in the molecular biology part of the measurement process. PCR
amplification is a well-known bias in the molecular biology part of the
...
performance \cite{wright2014exploiting}. Additional research is needed
before outlier features are attributed to mismatches in the primer
binding regions.
...
of false negative features is a potential explanation for DADA2 count
table's poor performance in the qualitative assessment and comparable
sparsity to the other pipelines despite having significantly fewer
features (Fig. \ref{fig:qualPlot} and Table \ref{tab:pipeQA}). The DADA2
...
\subsubsection*{Using Mixtures to Assess 16S rRNA Sequencing}
...
DNA in the unmixed samples using qPCR data. Using the 16S sequencing
...
inference method are not known resulting in an unaccounted for error
source.
...
assay'ssuitability for use in mixture studies. Digital PCR provides a
...
Our two-sample-titration dataset and assessment methods can be used to
evaluate and characterize bioinformatic pipelines and clustering
methods. The sequence dataset presented in this study can be processed
with any 16S rRNA bioinformatic pipeline. Our quantitative and
...
total feature abundance, number of features per samples, and total
features. The objective of any pipeline is to differentiate true
biological sequences from measurement process artifacts. In general,
based on our evaluation results we suggest using DADA2 for feature-level
abundance analysis, e.g.~differential abundance testing. While DADA2
performed poorly in our qualitative assessment, the pipeline performed
better in the quantitative assessment compared to the other pipelines.
Additionally, the DADA2 poor qualitative assessment results due to
false-negative features are unlikely to negatively impact feature-level
abundance analysis. When determining which pipeline to use for a study,
...
false-positive features. Feature-level results for any 16S rRNA
...
the proportion of prokaryotic DNA across titrations. To ensure the
...
unmixed samples after unmixed sample concentration was normalized to
...
and POST samples, titration bacterial DNA concentration was quantified
